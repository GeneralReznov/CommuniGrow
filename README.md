...
The CommuniGrow is a comprehensive web application designed to empower underserved communities through technology. Built with Flask and powered by AI (Groq with Gemini fallback), it provides accessible solutions across four core areas: Climate Action, Skills Development, Food Security, and Healthcare Access.
...
- **AI Integration**: Groq as the primary provider with Google Gemini as an automatic fallback
...
├── gemini.py               # Shared Groq/Gemini AI provider layer
...
### Groq + Google Gemini AI Integration

The platform uses Groq's `llama-3.3-70b-versatile` model as its primary provider and automatically retries with Google's Gemini 2.5 Flash when Groq is unavailable or returns an error. Both providers support the same AI-powered features:
...
4. **AI Service Integration**: Groq-first provider routing, Gemini fallback, and error handling
